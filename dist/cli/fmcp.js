...
import { exitCodeFor, formatFindings, runDoctor } from "./doctor.js";
import { fmcpHome, installRootFrom, isCloudEnv, logFilePath, pidFilePath } from "./paths.js";
import { fetchStatus, isProcessAlive, killPid, MAX_PORT, MIN_PORT, requestShutdown, scanBridges, sleep, } from "./probe.js";
    ...
    if (flags.has("fix")) {
        for (const x of findings) {
            if (!x.action)
                continue;
            try {
                fixResults.set(x, await x.action());
            }
            catch (e) {
                fixResults.set(x, `düzeltme başarısız: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
    }
    if (flags.has("json")) {
...
function describe(b) {
    if (b.kind !== "fmcp")
        return `:${b.port}  ${b.kind === "dead" ? "cevap vermiyor" : "FMCP değil"}${b.pid ? `  PID ${b.pid}` : ""}${b.command ? `  ${b.command.slice(0, 60)}` : ""}`;
    const s = b.status;
    if (!s)
        return `:${b.port}  FMCP (eski sürüm, /status yok)`;
...
async function cmdStatus(flags) {
    ...
    if (flags.has("json")) {
        out(JSON.stringify(bridges, null, 2));
        return 0;
    }
    if (bridges.length === 0) {
...
async function cmdStart(flags) {
    ...
    if (flags.has("foreground")) {
        ...
        if (port)
            process.env.FIGMA_PLUGIN_BRIDGE_PORT = port;
        await cmdServe();
        return 0;
    }
...
async function stopPort(port) {
    ...
    while (Date.now() < deadline) {
        if (!(await fetchStatus(port, "127.0.0.1", 300)))
            return true;
        await sleep(200);
    }
    if (before.pid && isProcessAlive(before.pid)) {
        killPid(before.pid);
        await sleep(500);
        if (isProcessAlive(before.pid))
            killPid(before.pid, "SIGKILL");
    }
...
async function cmdStop(flags) {
    ...
    if (!pid) {
        out("fmcp start ile başlatılmış bridge yok. Tüm bridge'ler için: fmcp stop --all");
        return 0;
    }
    if (!isProcessAlive(pid)) {
        ...
        out("Standalone bridge zaten kapalı (bayat pid dosyası silindi)");
        return 0;
    }
    const bridges = await scanBridges({ withPids: false });
    const mine = bridges.find((b) => b.status?.pid === pid);
    if (mine)
        await stopPort(mine.port);
    else {
        killPid(pid);
        await sleep(500);
        if (isProcessAlive(pid))
