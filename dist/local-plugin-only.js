...
import { resolveActiveDs as resolveActiveDsCache, getLibraryComponents as getLibraryComponentsCache, getLibraryTokens as getLibraryTokensCache, } from "./core/fmcp-cache-reader.js";
...
export async function main() {
    ...
    server.registerTool("figma_resolve_active_ds", {
        description: "Resolve the active design system from the FMCP user-local cache " +
    ...
    server.registerTool("figma_get_library_components", {
        ...
        annotations: { readOnlyHint: true, destructiveHint: false },
    }, safeToolHandler(async ({ libraryName, filter }) => {
        const ctx = await resolveActiveDsCache();
            ...
            return {
                content: [{
                        ...
                        text: JSON.stringify({
                            ...
                            _restFallbackHint: "No FMCP cache found. Fall back to Rule 24.1: figma_rest_api GET /v1/files/<LIBRARY_FILE_KEY>/components " +
            ...
            return {
                content: [{
                        ...
                        text: JSON.stringify({
                            success: false,
                            error: "FMCP cache missing",
                            _restFallbackHint: "No FMCP cache found. Fall back to figma_get_library_variables (plugin teamLibrary API) " +
    ...
    server.registerTool("figma_enumerate_published_components", {
        description: "List every published COMPONENT + COMPONENT_SET of a library via the Figma REST API. " +
        ...
        annotations: { readOnlyHint: true, destructiveHint: false },
    }, safeToolHandler(async ({ libraryFileKey }) => {
        ...
        for (const s of setsMeta) {
            if (s.name && s.key)
                items.push({ name: s.name, key: s.key, kind: "COMPONENT_SET", description: s.description ?? null });
        }
