...
import { buildStructureScript, buildOverridesScript, buildTokensScript, collectVariableIds, assembleContract, parseScriptResult, } from "./core/contract-extractor.js";
...
export async function main() {
    ...
    server.registerTool("figma_get_variables", {
        ...
        if (verbosity === "inventory") {
            ...
            const slim = raw.variables.map((v) => ({
                id: v.id,
                name: v.name,
    ...
    server.registerTool("figma_extract_contract", {
        description: "Extract a complete design contract JSON spec from a COMPONENT_SET (or a COMPONENT/INSTANCE inside one). " +
        ...
        let overrides = { ok: true, overrides: {} };
        if (structure.kind === "COMPONENT_SET" && structure.set?.id) {
            overrides = parseScriptResult(await conn.executeCodeViaUI(buildOverridesScript(structure.set.id), 30000), "overrides");
            if (!overrides.ok) {
        ...
        let tokens = { ok: true, tokens: {} };
        if (variableIds.length > 0) {
            tokens = parseScriptResult(await conn.executeCodeViaUI(buildTokensScript(variableIds.slice(0, MAX_TOKEN_IDS)), 30000), "tokens");
            if (!tokens.ok) {
    ...
    server.registerTool("figma_enumerate_published_components", {
        description: "List published COMPONENT + COMPONENT_SET of a library via the Figma REST API. " +
        ...
        for (const s of setsMeta) {
            if (s.name && s.key) {
                const item = { name: s.name, key: s.key, kind: "COMPONENT_SET" };
        ...
        for (const c of compsMeta) {
            if (!c.name || !c.key)
                continue;
            if (c.containing_component_set?.key)
                continue; // variant inside a set — covered by the set row
            const item = { name: c.name, key: c.key, kind: "COMPONENT" };
            if (includeDescription)
        ...
        const warnings = [];
        if (truncated) {
            warnings.push(`TRUNCATED: ${filteredByName.length} items matched, ${effectiveLimit} returned. ` +
    ...
    if (standalone) {
        logger.info({ port: actualPort, preferredPort: bridge.getPreferredPort(), autoIncremented: autoInc, pid: process.pid }, "F-MCP ATezer Bridge running STANDALONE (no MCP stdio client); WebSocket on port %s. Stop with `fmcp stop`.", actualPort);
        return;
    }
