...
export async function main() {
    ...
    const bridgeSessionMeta = () => ({
        ...
        parallelSessionsHint: "Birden fazla izole AI hattı (ör. Cursor + Claude veya iki sohbet) için her hatta ayrı MCP süreci ve plugin'de aynı port gerekir. Kullanıcı sohbette port yazdıysa figma_get_status ile expectedBridgePort göndererek eşleşmeyi doğrulayın.",
    });
    ...
    server.registerTool("figma_get_status", {
        ...
        inputSchema: {
            expectedBridgePort: z
                ...
                .max(5470)
                .optional()
                .describe("Kullanıcının sohbette belirttiği bridge portu. Bu MCP örneğinin portu ile karşılaştırılır; uyuşmazsa yanlış connector veya yanlış Figma plugin portu vardır."),
        },
        annotations: { readOnlyHint: true },
        ...
        const portCheck = expectedBridgePort !== undefined
            ? {
                expectedBridgePort,
                portMatchesExpected: expectedBridgePort === port,
                portMismatch: expectedBridgePort !== port
