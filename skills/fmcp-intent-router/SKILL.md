...
**⚠️ v1.9.9+ Hybrid Quick-Start (G26b):**

Target skill `generate-figma-screen` veya `apply-figma-design-system` ise VE user prompt'ta şu pattern'lerden biri varsa component discovery **hızlandırılır**:
- "seed component var" / "manuel yerleştirdim" / "X'i zaten ekledim" / "button/appbar sürükledim"

Bu durumda:
- ❌ G19 Step 3.1 Cache Populate **ATLA** (full scan gereksiz)
- ✅ **PRIMARY (v3.3+):** `figma_enumerate_published_components(libraryFileKey, filter)` ← registry'deki file key ile REST'ten canlı liste (dosya açıksa `figma_enumerate_library_components` daha hızlı). Tek call'da componentKey listesi.
- ✅ **FALLBACK (registry boş / REST token yok):** `figma_search_assets(currentPageOnly=false)` → mevcut instance'lardan `mainComponent.key` topla
- Discovery mini-mode: 1-3 call

**Component discovery öncelik sırası (Rule 24 köprüsü):**

1. **Rule 24.0 Live Enumerate** (`figma_enumerate_published_components` / `figma_enumerate_library_components` — v3.3+ ana yol) — G26c
...
### v1.9.9+ User Memory Uyum Kuralı (SERT — G21)

Kullanıcının `MEMORY.md`'sinde "verify assumptions" / "doğrula" / "sorma önce" / "assumption yapma" gibi bir feedback memory varsa:

- ❌ **YANLIŞ:** Her input için ayrı AskUserQuestion → 4 ayrı soru ile memory'ye uymaya çalışma. Bu **tool-use israfıdır**, Claude Desktop'ta session başına tool limit'e erken çarpar.
- ✅ **DOĞRU:** **Tek özet soru** (Adım 6 formatı) bu memory'yi TAM karşılar. "Verify assumption" gereksinimi **tek özet + onay** ile sağlanır. Kullanıcı özeti görüp [✅ Evet / ✏️ Değiştir / ❌ İptal] seçtiğinde "assumption verify" edilmiş sayılır.

**Priority resolution:** User memory kuralı **ne diyor** değil, **ne gerektiriyor** üzerinden yorumlanır. "Verify" gereksinimi tek özet onayla karşılanır; birden fazla AskUserQuestion çağrısı memory'ye fazladan uymak değil, verim kaybıdır.

**Örnek (doğru davranış):**
> User memory: "Do not assume anything, verify with me first."
> User prompt: "Ana-DS ile iOS ödeme sayfası" (device/mode belirsiz)
>
> ❌ YANLIŞ: 4 ayrı soru (Q1 device, Q2 mode, Q3 approach, Q4 ölçek)
> ✅ DOĞRU: Tek Adım 6 özet:
> "Anladım — Ana-DS + iOS payment. Netleştirmem gereken: device (iPhone 17 default), mode (Light default), approach (build-from-scratch + reference override), ölçek (tek scroll). Onaylıyor musun? [✅ Bunlarla başla / ✏️ Değiştir / ❌ İptal]"
...
SONUÇ:
  - Ana-DS Alt 3 sayfası referans alınmaz
  - Mevcut "Ana-DS Alt 3 — Anasayfa" frame'i açılmaz, screenshot alınmaz
  - ideation sayfası (ilham) kullanılabilir ama Ana-DS Alt 3 bölümü atlanır
```

**Anti-pattern:** Kullanıcı "Ana-DS Alt 3 atla" dedikten sonra onu screenshot alıp referans göstermek. v1.9.6'da bu explicit yasak — pre-flight'ta `exclude_references` kontrolü yapılır.
...
**⚠️ v1.9.9+ Kural (G22 — Single-Question Consolidation):**

Eksik input'lar varsa özet + eksik input'lar **AYNI AskUserQuestion** call'ında sunulur. Birden fazla AskUserQuestion çağrısı **YASAK** (G21 ile uyumlu). Örn:

> "Anladım: Ana-DS + iOS payment + build-from-scratch + reference override.
>
> Netleştirmem gereken:
>  (a) Device: iPhone 17 (default, Ana-DS tablosundan) / 16 Pro / 16 / 13-14
>  (b) Mode: Light (default) / Light+Dark
>
> Onaylıyor musun? [✅ Default'larla başla / ✏️ Değiştir (a veya b) / ❌ İptal]"

Kullanıcı "Değiştir" derse **sadece o seçim için** tek follow-up AskUserQuestion — yine tek call. Default'larla onay → Adım 7 execute.
...
```
Kullanıcı: "Ana-DS ile bir şey yap"

Router: 4 SKILL match (create, apply, audit, library).

AskUserQuestion:
"Ana-DS ile ne yapmak istiyorsun?
 (a) Yeni ekran oluştur (generate-figma-screen)
 (b) Mevcut ekrana Ana-DS uygula (apply-figma-design-system)
 (c) Ana-DS uyumu denet (audit-figma-design-system)
 (d) Ana-DS library'ye component ekle (generate-figma-library)"
```
...
Verdiğin node bir sayfa (PAGE). Altında şu adaylar var:
 ...
 1. Vadesiz TL — Ana-DS v10 (Hero Card)       — 4 child (temiz, hızlı clone)
 2. Vadesiz TL — Ana-DS v11 (Segmented)       — 5 child (temiz, hızlı clone)
 3. Vadesiz TL — Ana-DS v12 (Dark Header)     — 3 child (temiz, hızlı clone)
