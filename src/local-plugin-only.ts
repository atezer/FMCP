...
export async function main() {
	...
	server.registerTool(
		...
		{
			description:
				"[DEPRECATED v3.2+] Cache-based component lookup. Use figma_enumerate_library_components (live) instead. " +
		...
		safeToolHandler(async ({ libraryName, filter }: { libraryName: string; filter?: string }) => {
			...
			if (requiredLibraries.length > 1) {
				warnings.push(
					`REQUIRED_LIBRARIES: ${requiredLibraries.join(", ")} — ` +
	...
	server.registerTool(
		...
		{
			description:
				"[DEPRECATED v3.2+] Cache-based token lookup. Use figma_get_library_variables (live teamLibrary API) instead. " +
	...
	server.registerTool(
		...
		{
			description:
				"LIVE enumerate every published COMPONENT and COMPONENT_SET in a library Figma file. " +
	...
	server.registerTool(
		...
		{
			description:
				"Get Code Connect hints for components/instances: documentationLinks, componentKey, name, description. " +
	...
	server.registerTool(
		...
		{
			description:
				"High-level orchestrator that bundles component, token and design-context lookups in one call. " +
	...
	server.registerTool(
		...
		{
			description:
				"Create a prototype reaction between two nodes (source -> destination). " +
	...
	server.registerTool(
		"figma_get_prototype_connections",
		{
			description:
				"Read prototype reactions on a node subtree or the whole current page. " +
	...
	server.registerTool(
		...
		{
			description:
				"Create a variant state-change interaction on an INSTANCE/variant node (hover/press/focus -> variant). " +
	...
	server.registerTool(
		...
		{
			description:
				"Set a node's scroll behavior (Figma Prototype panel > Scroll behavior). " +
