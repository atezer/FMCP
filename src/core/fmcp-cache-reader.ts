 ...
 */
function parseActiveMd(text: string, baseDir: string): { libraryName: string | null; fileKey: string | null; cacheRootPath: string | null } {
...
function parseMetaSync(text: string): string | null {
	return matchLine(text, /\*\*Son başarılı sync:\*\*\s*([^\s]+)/);
}

function classifyFreshness(lastSyncIso: string | null): CacheStatus {
...
export async function resolveActiveDs(): Promise<ActiveDsContext> {
...
function sliceSections(text: string): ParsedSection[] {
 ...
 */
export async function getLibraryComponents(libraryName: string, filter?: string): Promise<LibraryComponent[]> {
	const ctx = await resolveActiveDs();
	if (!ctx.cacheRoot || ctx.libraryName !== libraryName) return [];
 ...
 */
export async function getLibraryTokens(libraryName: string, filter?: string): Promise<LibraryToken[]> {
	const ctx = await resolveActiveDs();
	if (!ctx.cacheRoot || ctx.libraryName !== libraryName) return [];
	const text = await readFileSafe(join(ctx.cacheRoot, "tokens.md"));
	if (!text) return [];

	const items: LibraryToken[] = [];
	const sections = sliceSections(text);
	for (const section of sections) {
		const heading = section.heading.trim();
		if (/^Collection Info$/i.test(heading)) continue;
		if (/^Mode Apply/i.test(heading)) continue;
		if (/^Text Styles/i.test(heading)) continue;
		if (/^Kullan/i.test(heading)) continue;
		...
		for (let i = 1; i < tableLines.length; i++) {
			...
			if (tokenIdx >= 0 && cells[tokenIdx]) {
				name = cells[tokenIdx];
				if (roleIdx >= 0 && cells[roleIdx]) name = `${cells[roleIdx]} — ${name}`;
			} else {
				name = cells[0] ?? "";
			}
			if (!name) continue;
...
function inferTokenType(heading: string): string {
	const lower = heading.toLowerCase();
	if (lower.includes("spacing")) return "spacing";
	if (lower.includes("radius")) return "radius";
	if (lower.includes("surface")) return "color";
	if (lower.includes("background")) return "color";
