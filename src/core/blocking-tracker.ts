...
export class BlockingTracker {
	private lastBlockingTimestamp = 0;
	private lastBlockingNodeIds: Set<string> = new Set();
	private lastBlockingCategories: string[] = [];
	...
	recordBlocking(nodeIds: string[], categories: string[] = []): void {
		...
		for (const id of nodeIds) {
			if (id && typeof id === "string") this.lastBlockingNodeIds.add(id);
		}
	...
	checkSuppression(executeCode: string): SuppressionCheckResult {
		...
		if (this.lastBlockingNodeIds.size === 0) return {};
		...
		return {
			...
			error:
				`❌ BLOCKING_SUPPRESSION_DETECTED: Node${matched.length > 1 ? "'lar" : ""} ${preview} icin son figma_execute'ta BLOCKING flag donmustu.${categoryHint} ` +
	 ...
	 */
	getState(): { nodeCount: number; categories: string[]; timestamp: number } {
		return {
