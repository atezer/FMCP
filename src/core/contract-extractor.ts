...
// ============================================================================
// Raw payload tipleri (plugin scriptlerinin döndürdüğü şekiller)
// ============================================================================
...
export interface RawNode {
	...
	mainComponent?: { id: string; name: string; parentName?: string | null };
...
export interface RawStructure {
	...
	set?: { id: string; name: string; key: string | null };
	fileKey?: string | null;
...
export interface RawOverrideEntry {
	...
	f: string;
	a?: { id?: string; v?: unknown; styleName?: string | null } | null;
	b?: { id?: string; v?: unknown; styleName?: string | null } | null;
}
 ...
 */
export function buildStructureScript(nodeId?: string): string {
	return `
 ...
 */
export function buildOverridesScript(setId: string): string {
	return `
 ...
 */
export function buildTokensScript(variableIds: string[]): string {
	return `
...
export function semanticPartName(node: Pick<RawNode, "name" | "type" | "w" | "h" | "mainComponent">): string {
	...
	if (node.type === "INSTANCE" && node.mainComponent) {
		...
		if (!GENERIC_NAME.test(base)) return toKebabCase(base);
	}
	if (!GENERIC_NAME.test(raw)) return toKebabCase(raw);
...
const FIELD_TO_CSS: Record<string, string> = {
	itemSpacing: "gap",
...
export function tokenRef(variableName: string): string {
...
function blendOver(fg: ParsedColor, bg: ParsedColor): ParsedColor {
	const a = fg.a;
	return {
		r: fg.r * a + bg.r * (1 - a),
		g: fg.g * a + bg.g * (1 - a),
...
export function computeContrastRatio(fgHex: string, bgHex: string): number | null {
	...
	const WHITE: ParsedColor = { r: 1, g: 1, b: 1, a: 1 };
...
const ELEMENT_HINTS: Array<[RegExp, string]> = [
...
export function inferElement(componentName: string): string {
	for (const [re, el] of ELEMENT_HINTS) {
		if (re.test(componentName)) return el;
	}
	return "div";
}
...
interface IndexedNode {
	node: RawNode;
	/** Structural index path ('' = root). */
	path: string;
	/** Görünen ad yolu (part adlarıyla). */
	displayPath: string;
}
...
function indexTree(tree: RawNode): { byPath: Map<string, IndexedNode>; partsOrder: IndexedNode[] } {
	...
	function walk(node: RawNode, path: string, displayPrefix: string, usedAtLevel: Set<string>, isTopLevel: boolean) {
		...
		for (let i = 0; i < children.length; i++) {
			...
			walk(child, childPath, displayPath, new Set<string>(), false);
		}
	}
...
export function normalizeProps(
...
): { props: ContractProp[]; states: string[]; orphans: string[] } {
	...
	for (const raw of rawProps) {
		...
		if (raw.type === "VARIANT") {
			...
			if (STATE_PROP_NAMES.has(lowerName)) {
				for (const opt of options) {
					...
					if (mapped && !states.includes(mapped)) states.push(mapped);
				}
			}
		...
		if (!referencedKeys.has(raw.key)) orphans.push(raw.key);
 ...
 */
export function parseVariantCombo(name: string): Record<string, string> {
	const out: Record<string, string> = {};
...
function collectReferencedKeys(tree: RawNode | undefined): Set<string> {
	const keys = new Set<string>();
	function walk(n: RawNode | undefined) {
		if (!n) return;
		if (n.propRefs) for (const v of Object.values(n.propRefs)) keys.add(v);
...
export function collectVariableIds(structure: RawStructure, overrides: RawOverrides): string[] {
	...
	function walkTree(n: RawNode | undefined) {
		if (!n) return;
		for (const id of Object.values(n.bound ?? {})) ids.add(id);
	...
	for (const group of Object.values(overrides.overrides ?? {})) {
		for (const entries of Object.values(group)) {
			for (const e of entries) {
				if (e.f === "textStyle") continue; // stil id'leri variable değil
				if (e.a?.id) ids.add(e.a.id);
				if (e.b?.id) ids.add(e.b.id);
			}
		}
	}
...
function formatOverrideValue(
	entry: RawOverrideEntry,
	tokensById: Record<string, RawTokenEntry>,
): string {
	const b = entry.b;
	if (!b) return "(none)";
	if (entry.f === "textStyle") return b.styleName ?? "(unknown style)";
	if (b.id) {
		const t = tokensById[b.id];
		return t ? tokenRef(t.name) : `{unresolved:${b.id}}`;
	}
	if (b.v !== undefined && b.v !== null) return String(b.v);
	return "(none)";
}

interface FormattedOverrides {
	[groupKey: string]: {
		[valueKey: string]: {
	...
	for (const [prop, group] of Object.entries(rawOverrides.overrides ?? {})) {
		...
		for (const [value, entries] of Object.entries(group)) {
			...
			for (const entry of entries) {
				...
				if (entry.f === "strokeWeight") css = "border-width";
				if (!css) continue;
...
function buildParts(
	...
	for (const { node, displayPath } of partsOrder) {
		...
		const entry: Record<string, unknown> = { attrs: { figmaName: node.name } };
		if (Object.keys(tokens).length > 0) entry.tokens = tokens;
		parts[key] = entry;
	}
	return parts;
}
...
function buildBaseSpecs(tree: RawNode, partsOrder: IndexedNode[]): Record<string, unknown> {
	...
	for (const { node, displayPath } of partsOrder) {
		...
		if (node.type === "TEXT") {
			...
			if (node.lineHeightUnit) c.lineHeightUnit = node.lineHeightUnit;
...
function buildA11y(
	...
	formattedOverrides: FormattedOverrides,
	anatomyRootTokens: Record<string, string>,
	partsOrder: IndexedNode[],
	tokensById: Record<string, RawTokenEntry>,
	defaultCombo: Record<string, string>,
): A11yPair[] {
	...
	let styleGroupKey: string | null = null;
	let stylePropName: string | null = null;
	...
	function pushPair(styleLabel: string, fgRef: string | undefined, bgRef: string | undefined) {
		...
		if (!fg || !bg || fg.type !== "COLOR" || bg.type !== "COLOR") return;
		...
		for (const mode of Object.keys(bg.values)) {
			...
			if (typeof fgHex !== "string" || typeof bgHex !== "string") continue;
			const ratio = computeContrastRatio(fgHex, bgHex);
			if (ratio !== null) ratios[mode] = ratio;
		}
