...
export class PluginBridgeServer {
	...
	private buildPortCandidateList(preferred: number): number[] {
		const p0 = Math.max(MIN_PORT, Math.min(MAX_PORT, preferred));
		const list: number[] = [];
		for (let p = p0; p <= MAX_PORT; p++) list.push(p);
		for (let p = MIN_PORT; p < p0; p++) list.push(p);
		return list;
	}

	private attemptListen(candidatePorts: number[], index: number): void {
	...
	private tryListenOne(port: number, candidatePorts: number[], index: number): void {
