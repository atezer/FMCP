...
export function isFmcpEntry(entry: { name: string; command?: string; args: string[]; url?: string }): boolean {
	if (/figma-mcp-bridge|fmcp/i.test(entry.name)) return true;
...
export function readFmcpEntries(file: string): McpServerEntry[] {
	const raw = JSON.parse(readFileSync(file, "utf8")) as { mcpServers?: Record<string, unknown> };
	const servers = raw.mcpServers && typeof raw.mcpServers === "object" ? raw.mcpServers : {};
	const out: McpServerEntry[] = [];
	for (const [name, value] of Object.entries(servers)) {
		if (!value || typeof value !== "object") continue;
		const v = value as { command?: unknown; args?: unknown; url?: unknown; env?: unknown };
		const entry: McpServerEntry = {
			...
			env: v.env && typeof v.env === "object" ? Object.fromEntries(Object.entries(v.env as Record<string, unknown>).filter(([, x]) => typeof x === "string")) as Record<string, string> : {},
		};
		if (isFmcpEntry(entry)) out.push(entry);
	}
	return out;
}
...
export type EntryVerdict =
	...
	| { level: "warn"; detail: string; fix: string }
	| { level: "fail"; detail: string; fix: string };
 ...
 */
export function judgeEntry(entry: McpServerEntry, installRoot: string, opts: { fileExists?: (p: string) => boolean } = {}): EntryVerdict {
