...
import {
	type BridgeInfo, isProcessAlive, killPid, listFmcpProcesses, MAX_PORT, MIN_PORT,
...
export async function runDoctor(ctx: DoctorContext): Promise<Finding[]> {
	const f: Finding[] = [];

	// 1. Node
	const major = nodeMajor(ctx.nodeVersion);
	if (major < 18) f.push({ check: "node", level: "fail", title: `Node ${ctx.nodeVersion} çok eski`, fix: "Node 20 LTS kurun (https://nodejs.org)" });
	else if (major < 20) f.push({ check: "node", level: "warn", title: `Node ${ctx.nodeVersion} çalışır, 20+ önerilir`, fix: "Node 20 LTS'e güncelleyin" });
	else f.push({ check: "node", level: "ok", title: `Node ${ctx.nodeVersion}` });
	...
	if (fmcp.length === 0) {
		...
		for (const b of fmcp) f.push(describeBridge(b, ctx));
	}
	...
	const listeningPids = new Set(fmcp.map((b) => b.pid).filter((p): p is number => p !== null));
	...
	if (procs.length === 0 && stale.length === 0) {
		...
		if (procs.length > 0) {
			f.push({
				...
				action: async () => {
					let n = 0;
					for (const p of procs) if (killPid(p.pid)) n++;
					await sleep(500);
					for (const p of procs) if (isProcessAlive(p.pid)) killPid(p.pid, "SIGKILL");
		...
		if (stale.length > 0) {
			f.push({
				...
				action: async () => {
					let n = 0;
					for (const b of stale) if (await requestShutdown(b.port)) n++;
...
function describeBridge(b: BridgeInfo, ctx: DoctorContext): Finding {
	const s = b.status;
	if (!s) return { check: "ports", level: "warn", title: `Port ${b.port}: FMCP bridge (eski sürüm, /status yok)`, fix: "Sunucuyu güncelleyin: npm run update" };
...
const ICON: Record<Level, string> = { ok: "✅", info: "ℹ️ ", warn: "⚠️ ", fail: "❌" };

export function formatFindings(findings: Finding[], fixResults: Map<Finding, string> = new Map()): string {
	const lines: string[] = [];
	for (const x of findings) {
		lines.push(`${ICON[x.level]} ${x.title}`);
		if (x.detail) for (const d of x.detail.split("\n")) lines.push(`     ${d}`);
		if (x.fix && x.level !== "ok") lines.push(`     → ${x.fix}`);
